...
{
  public class RBScriptCali : MonoBehaviour
  {
    ...
    void Update()
    {
      ...
      if (ORot.Result == Result.Success && OTran.Result == Result.Success && !OTran.Occluded )
      {
        ...
        Root.localPosition = new Vector3((-(float)OTran.Translation[0] - Root.localScale.x/2) * 0.001f, ((float)OTran.Translation[2]-Root.localScale.y/2) * 0.001f, -((float)OTran.Translation[1]-Root.localScale.z/2) * 0.001f);
